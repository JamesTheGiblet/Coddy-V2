...
class CodeGenerator:
    ...
    async def initialize(self):
        ...
        """
        self.logger.info("CodeGenerator initialized.") # Now correctly awaited
    ...
    def _extract_code_from_markdown(self, content: str) -> str:
        ...
        """
        self.logger.debug("Attempting to extract code from markdown.")
        # Find the start of a code block
        start_marker = content.find('```')
        if start_marker == -1:
            self.logger.debug("No markdown code block found. Returning original content.")
        ...
        if end_marker == -1:
            ...
            self.logger.warning("Markdown code block started but not ended. Assuming rest is code.")
        ...
        extracted_code = code_block[first_newline + 1:].strip() if first_newline != -1 else code_block.strip()
        self.logger.debug("Code extracted successfully.")
    ...
    def _is_valid_python_code(self, code: str) -> bool:
        ...
        """
        self.logger.debug("Validating Python code syntax.")
        try:
            ast.parse(code)
            self.logger.debug("Python code syntax is valid.")
            return True
        except SyntaxError as e:
            self.logger.debug(f"Generated code has a syntax error: {e}", exc_info=True) # Added exc_info
            return False
    ...
    async def _generate_and_validate_code(self, initial_prompt: str, user_profile: Optional[Dict[str, Any]]) -> str:
        ...
        """
        self.logger.info("Initiating code generation and validation.")
        # First attempt
        ...
        if not self._is_valid_python_code(generated_code):
            self.logger.warning("Initial code generation failed syntax check. Attempting to self-correct.")
            correction_prompt = (
            ...
            # Final check
            if not self._is_valid_python_code(generated_code):
                self.logger.error("Self-correction also failed syntax check. Returning faulty code.", exc_info=True) # Added exc_info
    ...
    async def generate_code(self, prompt: str, output_file: Optional[str] = None, 
        ...
        try:
            ...
            if output_file:
                ...
                self.logger.info(f"Generated content saved to {output_file} under '{category}' for project '{project_name}'.")
    ...
    async def generate_code_fix(self, file_path: str, output_file: Optional[str] = None, 
        ...
        """
        self.logger.info(f"Generating code fix for file: {file_path}")
        ...
        try:
            ...
            self.logger.info("Code fix generation complete.")
            ...
            if output_file:
                ...
                self.logger.info(f"Generated code fix saved to {output_file} under 'fixes' for project '{project_name}'.")
        except Exception as e:
            self.logger.error(f"Error during code fix generation: {e}", exc_info=True)
